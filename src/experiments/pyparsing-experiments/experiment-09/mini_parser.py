...
# - support matrix expressions using square '[' ']' brackets
...
FUNCTION_CALL    = (FUNCTION_NAME + LPAR + EXPR + RPAR).setParseAction(pushFunction)

ROW_WITH_COMMAS  = EXPR + OneOrMore(COMMA + EXPR) | EXPR + COMMA | EXPR
ROW_WITH_SEMIS   = ROW_WITH_COMMAS + SEMI | SEMI
ROW_WITH_SPACES  = EXPR + OneOrMore(EXPR) | EXPR
ROW              = ROW_WITH_SEMIS | ROW_WITH_COMMAS | ROW_WITH_SPACES
MATRIX           = LBRACKET + ZeroOrMore(ROW) + RBRACKET

SUB_1            = ( NUMBER | MATRIX | FUNCTION_CALL | ID_REF ).setParseAction(pushFirst)
...
ASSIGNMENT       = (ASSIGNED_ID + EQUALS + EXPR).setParseAction(store_assignment)
