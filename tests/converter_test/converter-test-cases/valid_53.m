function [Tsim SimData] = valid_53
...
[Tsim, SimData] = ode15s(@rhs, tspan, x0, [], k, I);
...
function dxdt = rhs(t,x, k, I)
% three component incoherent feed-forward network
dxdt = [k(1)*I*(1-x(1))/(1-x(1)+k(7)) - k(2)*x(1)/(x(1)+k(8));k(3)*x(1)*(1-x(2))/(1-x(2)+k(9)) - k(4)*x(2)/(x(2)+k(10));k(5)*x(1)*(1-x(3))/(1-x(3)+k(11)) - k(6)*x(2)*x(3)/(x(3)+k(12))]
end
end
