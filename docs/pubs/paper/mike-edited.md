...
MOCCASIN's GUI interface is implemented using wxPython, a cross-platform GUI toolkit (Rappin and Dunn, 2006).  The interface provides a straightforward way for users to input MATLAB files, set MOCCASIN options such as the type of output (SBML or XPP), view the resulting output, and save the converted file.
